...
public class SubscriptionDataUtils {
    ...
    public static String addMseGrayTagsToSql92Expression(String originSubData) {
        String rebuildOriginSubData = originSubData;
        if (!StringUtils.isBlank(rebuildOriginSubData)) {
            rebuildOriginSubData = removeMseGrayTagFromOriginSubData(rebuildOriginSubData);
        }
        String sql92Expression = buildSql92Expression();
        if (StringUtils.isBlank(sql92Expression)) {
            return rebuildOriginSubData;
        } else {
            return StringUtils.isBlank(rebuildOriginSubData)
                    ? sql92Expression : rebuildOriginSubData + " and " + sql92Expression;
        }
    }
