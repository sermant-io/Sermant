...
import io.sermant.mq.grayscale.config.MessageGrayBase;
...
public class MqGrayscaleConfigUtils {
     ...
     */
    public static final String MICRO_SERVICE_GRAY_TAG_KEY = "micro_service_gray_tag";

    /**
     * traffic message tag key
     */
    public static final String MICRO_TRAFFIC_GRAY_TAG_KEY = "micro_traffic_gray_tag";

    /**
     * config cache key
     */
    public static final String CONFIG_CACHE_KEY = "mqGrayConfig";

    /**
     * exclude tag change flag
     */
    private static boolean mqExcludeTagChangeFlag = false;

    private static final Map<String, String> MICRO_SERVICE_PROPERTIES = new HashMap<>();

    private static final Map<String, MqGrayscaleConfig> CONFIG_CACHE = new ConcurrentHashMap<>();

    private static final String PERCENT_SIGN = "%";

    private static final long AUTO_CHECK_DELAY_TIME = 30L;
    ...
    public static void modifyExcludeTags(Set<String> excludeTags) {
        ...
        mqExcludeTagChangeFlag = true;
        if (CONFIG_CACHE.get(CONFIG_CACHE_KEY).getBase() == null) {
            CONFIG_CACHE.get(CONFIG_CACHE_KEY).setBase(new MessageGrayBase());
        }
    ...
    public static boolean isMqExcludeTagChangeFlag() {
        return mqExcludeTagChangeFlag;
    }

    public static void setMqExcludeTagChangeFlag(boolean mqExcludeTagChangeFlag) {
        MqGrayscaleConfigUtils.mqExcludeTagChangeFlag = mqExcludeTagChangeFlag;
    }
}
