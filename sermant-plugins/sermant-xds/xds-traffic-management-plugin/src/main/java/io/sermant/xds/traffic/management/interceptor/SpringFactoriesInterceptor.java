...
/**
 * SpringFactoriesLoader interception
 *
 * @author lilai
 * @since 2025-06-04
 */
public class SpringFactoriesInterceptor implements Interceptor {
    ...
    public SpringFactoriesInterceptor() {
        for (ClassInjectDefine define : ServiceLoader.load(ClassInjectDefine.class, this.getClass().getClassLoader())) {
            if (define.plugin() == Plugin.XDS_PLUGIN) {
        ...
        if (hasMethodLoadSpringFactories == null) {
            try {
                ...
                LoggerFactory.getLogger()
                        .fine("It is low version spring framework, class will be injected by loadFactoryNames");
    ...
    private void injectConfigurationsWithLowVersion(Object result, String factoryName) {
        ...
        if (result instanceof List) {
            ...
            CLASS_DEFINES.forEach(classInjectDefine -> {
                final List<String> injectClasses = service.injectConfiguration(factoryName, null, classInjectDefine,
                        contextClassLoader);
    ...
    private void injectConfigurations(Object result) {
        ...
        if (result instanceof Map) {
            ...
            LOGGER.warning(String.format(Locale.ENGLISH, "Can not inject ssl configuration! the type of cache is [%s]",
