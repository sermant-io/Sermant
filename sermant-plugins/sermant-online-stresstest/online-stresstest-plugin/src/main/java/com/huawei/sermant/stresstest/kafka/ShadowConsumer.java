...
public class ShadowConsumer<K, V> implements Consumer<K, V> {
     ...
     * 构造方法
     *
     * @param originalConsumer originalConsumer
     */
    ...
    public long position(TopicPartition partition) {
        return Tester.isTestTopic(partition.topic()) ? testConsumer.position(partition)
            : originalConsumer.position(partition);
    }

    @Override
    public long position(TopicPartition partition, Duration timeout) {
        return Tester.isTestTopic(partition.topic()) ? testConsumer.position(partition, timeout)
            : originalConsumer.position(partition, timeout);
    }

    @Override
    public OffsetAndMetadata committed(TopicPartition partition) {
        return Tester.isTestTopic(partition.topic()) ? testConsumer.committed(partition)
            : originalConsumer.committed(partition);
    }

    @Override
    public OffsetAndMetadata committed(TopicPartition partition, Duration timeout) {
        return Tester.isTestTopic(partition.topic()) ? testConsumer.committed(partition, timeout)
            : originalConsumer.committed(partition, timeout);
    }
    ...
    public List<PartitionInfo> partitionsFor(String topic, Duration timeout) {
        return Tester.isTestTopic(topic) ? testConsumer.partitionsFor(topic, timeout)
            : originalConsumer.partitionsFor(topic, timeout);
    }
