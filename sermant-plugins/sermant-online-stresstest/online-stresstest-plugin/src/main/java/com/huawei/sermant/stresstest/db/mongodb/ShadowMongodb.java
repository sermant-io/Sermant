...
public class ShadowMongodb {
    ...
    public MongoDatabase getShadowDatabaseFromClient(Object mongoClient, String database) {
        ...
        if (mongoClient instanceof MongoClient) {
            ...
            return new MongoDatabaseImpl(info.getDatabase(),
                (CodecRegistry)Objects
                    .requireNonNull(Reflection.invokeDeclared("getCodecRegistry", client).orElse(null)),
                ...
                (OperationExecutor)Objects
                    .requireNonNull(Reflection.invokeDeclared("createOperationExecutor", client).orElse(null)));
            ...
            LOGGER.severe(String.format(Locale.ROOT, "Unknown mongoClient %s", mongoClient.getClass().getName()));
        }
        return null;
    }
    ...
    private MongoClientImpl newShadowNewMongoClient(MongoClientImpl client, String database, MongoSourceInfo info) {
        if (SHADOW_CLIENTS.containsKey(database)) {
            ...
            MongoClientSettings.Builder clientBuilders = Reflection.getDeclaredValue("settings", client)
                .map(settings -> getFromSettings(settings, info, database)).orElse(MongoClientSettings.builder());
    ...
    private MongoCredential createShadowCredential(MongoCredential credential, String originDatabase,
        MongoSourceInfo info) {
