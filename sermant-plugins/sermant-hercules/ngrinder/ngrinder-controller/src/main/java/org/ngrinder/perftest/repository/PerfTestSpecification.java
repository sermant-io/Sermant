...
public abstract class PerfTestSpecification {
	...
	public static Specification<PerfTest> perfSceneEqual(final PerfScene perfScene) {
		return new Specification<PerfTest>() {
			@Override
			public Predicate toPredicate(Root<PerfTest> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder) {
				return criteriaBuilder.or(criteriaBuilder.equal(root.get("perfScene"), perfScene));
			}
		};
	}
}
