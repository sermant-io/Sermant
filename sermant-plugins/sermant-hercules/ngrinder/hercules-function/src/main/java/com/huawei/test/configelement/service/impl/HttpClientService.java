...
public class HttpClientService extends ConfigElement<HttpClientConfig> implements HttpClientServiceInterface {
    ...
    public HttpClientService(HttpClientConfig config){
        initConfig(config);
    }
    ...
    public void initConfig(HttpClientConfig config) {
        // 设置请求的默认配置
        RequestConfig requestConfig = setRequestConfig(config);
        /*
         * 设置默认保持长连接
         */
    ...
    private HttpResponse getHttpResponse(CloseableHttpClient closeableHttpClient, HttpRequestBase requestBase){
        try {
            ...
            if (HttpStatus.SC_OK == statusLine.getStatusCode()){
                ...
                throw new ClientProtocolException("Unexpected response status: " + statusLine.getStatusCode());
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
