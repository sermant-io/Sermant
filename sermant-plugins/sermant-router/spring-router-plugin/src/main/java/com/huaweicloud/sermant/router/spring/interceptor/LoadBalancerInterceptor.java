...
public class LoadBalancerInterceptor extends AbstractInterceptor {
    ...
    public ExecuteContext before(ExecuteContext context) {
        ...
        if (requestData == null) {
            arguments[0] = loadBalancerService.getZoneInstances(serviceId, instances,
                routerConfig.isEnabledSpringZoneRouter());
            return context;
        }
        List<Object> list = loadBalancerService.getTargetInstances(serviceId, instances, requestData.getPath(),
            requestData.getHeader());
        arguments[0] = loadBalancerService.getZoneInstances(serviceId, list, routerConfig.isEnabledSpringZoneRouter());
        return context;
    }
