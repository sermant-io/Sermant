...
public class RunnableAndCallableWrapper<T> extends AbstractThreadWrapper<T> implements Runnable, Callable<T> {
     ...
     */
    public RunnableAndCallableWrapper(Runnable runnable, Callable<T> callable, RequestHeader requestHeader,
            RequestData requestData, boolean cannotTransmit) {
        super(runnable, callable, requestHeader, requestData, cannotTransmit);
    }
}