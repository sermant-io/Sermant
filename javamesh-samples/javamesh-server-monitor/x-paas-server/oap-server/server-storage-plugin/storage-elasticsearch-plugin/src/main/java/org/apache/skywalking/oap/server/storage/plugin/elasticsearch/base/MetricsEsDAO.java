...
public class MetricsEsDAO extends EsDAO implements IMetricsDAO {
    ...
    public InsertRequest prepareBatchInsert(Model model, Metrics metrics) throws IOException {
        ...
        // huawei update.无损演练：修改复制标签存储索引
        if (metrics.id().endsWith(BaseReplicator.COPY_FLAG)) {
            modelName = modelName.replace(model.getName(),  model.getName() + BaseReplicator.COPY_FLAG);
        }
