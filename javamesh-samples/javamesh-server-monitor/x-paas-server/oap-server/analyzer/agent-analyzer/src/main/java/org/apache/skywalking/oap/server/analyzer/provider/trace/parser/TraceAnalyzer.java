    ...
    public void doAnalysis(SegmentObject segmentObject) {
        ...
        try {
            ...
            segmentObject.getSpansList().forEach(spanObject -> {
                // huawei update, change log: 修复兼容zipkin时，spanId会存在不为0导致segment解析补全的问题
                if (spanObject.getSpanId() == 0 || segmentObject.getSpansList().size() == 1) {
            ...
            // huawei update.无损演练：cpm/sla指标复制
            DrillThreadLocal.set(SegmentObjectReplicator.getLabelMetricValue(segmentObject));
