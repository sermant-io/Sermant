...
import com.huawei.apm.core.drill.SegmentObjectReplicator;
import com.huawei.oap.server.analyzer.SpanUtil;
...
public class MultiScopesAnalysisListener implements EntryAnalysisListener, ExitAnalysisListener, LocalAnalysisListener {
    ...
    public void parseEntry(SpanObject span, SegmentObject segmentObject) {
        ...
        if (span.getRefsCount() > 0) {
            for (int i = 0; i < span.getRefsCount(); i++) {
                ...
                // Huawei update, change log: 信息不全时，不解析span
                if (PLACEHOLDER.equals(reference.getParentService())
                    || PLACEHOLDER.equals(reference.getParentServiceInstance())
                    || PLACEHOLDER.equals(reference.getParentEndpoint())) {
                    continue;
                }
                ...
                sourceBuilder.setCopy(SegmentObjectReplicator.getCopy(segmentObject));
    ...
    public void parseExit(SpanObject span, SegmentObject segmentObject) {
        ...
        if (networkAddressAlias == null) {
            ...
            // huawei update, log: complete destService for sourceBuilder
            if (segmentObject.getTraceId().length() <= SpanUtil.TRACE_ID_LENGTH) {
                switch (span.getSpanLayer()) {
                    case MQ:
                        sourceBuilder.setDestNodeType(NodeType.MQ);
                        sourceBuilder.setDestServiceInstanceName(networkAddress);
                        sourceBuilder.setDestServiceName(networkAddress);
                        break;
                    case Cache:
                        sourceBuilder.setDestNodeType(NodeType.Cache);
                        sourceBuilder.setDestServiceInstanceName(networkAddress);
                        sourceBuilder.setDestServiceName(networkAddress);
                        break;
                    case Database:
                        sourceBuilder.setDestNodeType(NodeType.Database);
                        sourceBuilder.setDestServiceInstanceName(networkAddress);
                        sourceBuilder.setDestServiceName(networkAddress);
                        break;
                    default:
                        span.getTagsList().forEach(pair -> {
                            if (SpanUtil.DOWNSTREAM_SERVICE.equals(pair.getKey())) {
                                sourceBuilder.setDestServiceName(pair.getValue());
                            }
                        });
                        sourceBuilder.setDestNodeType(NodeType.Normal);
                }
            ...
            if (StringUtil.isEmpty(sourceBuilder.getDestServiceName())) {
                return;
            }
        } else {
            /*
        ...
        if (sourceBuilder.getType().equals(RequestType.DATABASE)) {
            ...
            // huawei update.无损演练：添加复制标签
            statement.setCopy(SegmentObjectReplicator.getCopy(segmentObject));
        ...
        span.getTagsList().forEach(tag -> {
            switch (tag.getKey()) {
                case LOGIC_ENDPOINT:
                    ...
                    sourceBuilder.setResponseCode(Const.NONE);
                    sourceBuilder.setCopy(SegmentObjectReplicator.getCopy(segmentObject));// huawei update.无损演练：添加复制标签
