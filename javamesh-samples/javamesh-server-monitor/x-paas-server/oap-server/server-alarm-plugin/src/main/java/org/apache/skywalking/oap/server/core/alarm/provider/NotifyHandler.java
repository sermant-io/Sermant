...
import com.huawei.skywalking.oap.server.core.alarm.callback.KafkaCallBack;
...
public class NotifyHandler implements MetricsNotify {
    ...
    public void notify(Metrics metrics) {
        ...
        metaInAlarm.setCopy(BaseReplicator.getCopyFlagValue(metrics));
    ...
    public void init(AlarmCallback... callbacks) {
        ...
        // update by huawei 20210727
        if (alarmSettings != null && alarmSettings.isOpenKafkaCallBack()) {
            allCallbacks.add(new KafkaCallBack(kafkaProducer, alarmSettings));
        }
