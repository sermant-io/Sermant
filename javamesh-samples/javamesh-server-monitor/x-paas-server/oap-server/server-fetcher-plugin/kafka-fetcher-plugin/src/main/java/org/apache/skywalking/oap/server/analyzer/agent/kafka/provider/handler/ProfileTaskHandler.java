...
public class ProfileTaskHandler implements KafkaHandler {
    ...
    public void handle(final ConsumerRecord<String, Bytes> record) {
        try {
            ...
            if (CoreModuleConfig.losslessDrillSwitchStatus && snapshot.getCopy() == BaseReplicator.TO_COPY) {
                ProfileThreadSnapshotRecord copyRecord = BaseReplicator.deepCopy(snapshotRecord, ProfileThreadSnapshotRecord.class);
                copyRecord.setCopy(snapshot.getCopy());
                RecordStreamProcessor.getInstance().in(copyRecord);
            }
