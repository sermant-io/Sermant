...
public class ManagementServiceHandler extends ManagementServiceGrpc.ManagementServiceImplBase implements GRPCHandler {
    ...
    public void reportInstanceProperties(final InstanceProperties request,
        ...
        // huawei update.无损演练：复制标签值
        if (CoreModuleConfig.losslessDrillSwitchStatus && request.getCopy() == BaseReplicator.TO_COPY) {
            ServiceInstanceUpdate copyServiceInstanceUpdate = BaseReplicator.deepCopy(serviceInstanceUpdate, ServiceInstanceUpdate.class);
            copyServiceInstanceUpdate.setCopy(request.getCopy());
            sourceReceiver.receive(copyServiceInstanceUpdate);
        }
    ...
    public void keepAlive(final InstancePingPkg request, final StreamObserver<Commands> responseObserver) {
        ...
        if (CoreModuleConfig.losslessDrillSwitchStatus && request.getCopy() == BaseReplicator.TO_COPY) {
            ...
            ServiceMeta copyServiceMeta = BaseReplicator.deepCopy(serviceMeta, ServiceMeta.class);
            copyServiceMeta.setCopy(request.getCopy());
            sourceReceiver.receive(copyServiceMeta);
        }
