...
import com.huawei.apm.core.drill.JVMMetricReplicator;
...
public class JVMMetricReportServiceHandler extends JVMMetricReportServiceGrpc.JVMMetricReportServiceImplBase implements GRPCHandler {
    ...
    public void collect(JVMMetricCollection request, StreamObserver<Commands> responseObserver) {
        ...
        builder.getMetricsList().forEach(jvmMetric -> {
            jvmSourceDispatcher.sendMetric(builder.getService(), builder.getServiceInstance(), jvmMetric, BaseReplicator.NO_COPY);
        });

        // huawei update.无损演练：修改复制标签值
        if (CoreModuleConfig.losslessDrillSwitchStatus && builder.getCopy() == BaseReplicator.TO_COPY) {
            JVMMetricReplicator.copyModification(builder);
            builder.getMetricsList().forEach(jvmMetric -> {
                jvmSourceDispatcher.sendMetric(builder.getService(), builder.getServiceInstance(), jvmMetric, BaseReplicator.TO_COPY);
            });
        }
