        ...
        return new StreamObserver<SegmentObject>() {
            ...
            public void onNext(SegmentObject segment) {
                ...
                try {
                    ...
                    SegmentObjectReplicator.addCashSegmentObjectMap(segment, BaseReplicator.NO_COPY, null);
                    segmentParserService.send(segment);
                    SegmentObjectReplicator.removeCashSegmentObjectMap(segment);

                    // huawei update.无损演练：修改复制标签值
                    SegmentObject copySegment = SegmentObjectReplicator.copyModification(segment);
                    if (copySegment != null) {
                        segmentParserService.send(copySegment);
                        SegmentObjectReplicator.removeCashSegmentObjectMap(copySegment);
                    }
                } catch (Exception e) {
                    errorCounter.inc();
                } finally {
