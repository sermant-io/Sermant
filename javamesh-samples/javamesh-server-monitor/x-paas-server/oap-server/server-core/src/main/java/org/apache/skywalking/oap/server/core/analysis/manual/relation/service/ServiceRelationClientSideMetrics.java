...
public class ServiceRelationClientSideMetrics extends Metrics {
    ...
    @Override
    public String id() {
        String splitJointId = getTimeBucket() + Const.ID_CONNECTOR + entityId;

        // huawei update.无损演练：添加复制标签
        if (this.copy == BaseReplicator.TO_COPY) {
            splitJointId = splitJointId + BaseReplicator.COPY_FLAG;
        }
        return splitJointId;
    }
    ...
    public Metrics toHour() {
        ...
        metrics.setCopy(getCopy()); // huawei update.无损演练：添加复制标签
        return metrics;
    }
