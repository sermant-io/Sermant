...
public class AnnotationScan {
    ...
    public void scan() throws IOException, StorageException {
        ...
        // update huawei APM issue #46 change log: 添加顶级包注解扫描，兼容scope类移动到com.huawei.*包  --start
        ImmutableSet<ClassPath.ClassInfo> extensionClasses = classpath.getTopLevelClassesRecursive("com.huawei");
        List<ClassPath.ClassInfo> classList = classes.stream().collect(Collectors.toList());
        classList.addAll(extensionClasses);
//        for (ClassPath.ClassInfo classInfo : classes) {
        for (ClassPath.ClassInfo classInfo : classList) {
            // update huawei APM issue #46 change log: 添加顶级包注解扫描，兼容scope类移动到com.huawei.*包  --end
