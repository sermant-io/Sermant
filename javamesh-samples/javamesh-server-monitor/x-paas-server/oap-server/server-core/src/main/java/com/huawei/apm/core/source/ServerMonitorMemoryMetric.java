/*
 * Copyright (c) Huawei Technologies Co., Ltd. 2021-2021. All rights reserved.
 */

package com.huawei.apm.core.source;

import com.huawei.apm.core.define.ExtScopeDefine;

import lombok.Getter;
import lombok.Setter;

import org.apache.skywalking.oap.server.core.source.DefaultScopeDefine;
import org.apache.skywalking.oap.server.core.source.ScopeDeclaration;
import org.apache.skywalking.oap.server.core.source.ScopeDefaultColumn;
import org.apache.skywalking.oap.server.core.source.Source;

/**
 * 服务器物理内存的实体类
 *
 * @author zhengbin zhao
 * @since 2021-02-25
 */
@ScopeDeclaration(
        id = ExtScopeDefine.SERVER_MONITOR_MEMORY_METRIC,
        name = "ServerMonitorMemoryMetric",
        catalog = DefaultScopeDefine.SERVICE_INSTANCE_CATALOG_NAME)
@ScopeDefaultColumn.VirtualColumnDefinition(
        fieldName = "entityId",
        columnName = "entity_id",
        isID = true,
        type = String.class)
public class ServerMonitorMemoryMetric extends Source {
    @Getter
    @Setter
    private String id;
    @Getter
    @Setter
    @ScopeDefaultColumn.DefinedByField(columnName = "name", requireDynamicActive = true)
    private String name;
    @Getter
    @Setter
    @ScopeDefaultColumn.DefinedByField(columnName = "service_name", requireDynamicActive = true)
    private String serviceName;
    @Getter
    @Setter
    @ScopeDefaultColumn.DefinedByField(columnName = "service_id")
    private String serviceId;
    @Getter
    @Setter
    private long memoryTotal;
    @Getter
    @Setter
    private long swapCached;
    @Getter
    @Setter
    private long cached;
    @Getter
    @Setter
    private long buffers;
    @Getter
    @Setter
    private long memoryUsed;

    @Override
    public int scope() {
        return ExtScopeDefine.SERVER_MONITOR_MEMORY_METRIC;
    }

    @Override
    public String getEntityId() {
        return String.valueOf(id);
    }

    @Getter
    @Setter
    @ScopeDefaultColumn.DefinedByField(columnName = "copy", requireDynamicActive = false)
    private int copy;// huawei update.无损演练新增属性
}
