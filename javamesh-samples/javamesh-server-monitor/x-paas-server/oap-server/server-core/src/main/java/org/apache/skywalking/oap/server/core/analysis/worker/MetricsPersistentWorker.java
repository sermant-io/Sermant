...
public class MetricsPersistentWorker extends PersistenceWorker<Metrics> {
    ...
    private void flushDataToStorage(List<Metrics> metricsList,
        ...
        // huawei update.无损演练：将复制标签与原标签分开处理
        List<Metrics> metrics = new ArrayList<>();
        List<Metrics> copyMetrics = new ArrayList<>();
        for (Metrics metric : metricsList) {
            if (BaseReplicator.getCopyFlagValue(metric) == BaseReplicator.TO_COPY) {
        ...
        if (metrics.size() > 0) {
            flushDataToStorageItem(metrics, prepareRequests);
        }
        if (copyMetrics.size() > 0) {
            flushDataToStorageItem(copyMetrics, prepareRequests);
        }
        metricsList.clear();
    }
    private void flushDataToStorageItem(List<Metrics> metricsList,
                                    List<PrepareRequest> prepareRequests) {
        try {
