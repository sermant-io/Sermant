...
public enum DataTTLKeeperTimer {
    ...
    private void execute(Model model) {
        try {
            ...
            IHistoryDeleteDAO service = moduleManager.find(StorageModule.NAME)
                    .provider()
                    .getService(IHistoryDeleteDAO.class);
            service.deleteHistory(model, Metrics.TIME_BUCKET,
                    model.isRecord() ? moduleConfig.getRecordDataTTL() : moduleConfig.getMetricsDataTTL()
            );

            // huawei update.无损演练开关：是否需要删除复制索引
            if (CoreModuleConfig.losslessDrillSwitchStatus) {
                // huawei update.无损演练：启动时删除已过期的复制标签索引
                Model copyModel = new Model(
                        model.getName() + BaseReplicator.COPY_FLAG, model.getColumns(), model.getExtraQueryIndices(), model.getScopeId(),
                        model.getDownsampling(), model.isRecord(), model.isSuperDataset()
                );
                service.deleteHistory(copyModel, Metrics.TIME_BUCKET,
                        copyModel.isRecord() ? moduleConfig.getRecordDataTTL() : moduleConfig.getMetricsDataTTL()
                );
            }
