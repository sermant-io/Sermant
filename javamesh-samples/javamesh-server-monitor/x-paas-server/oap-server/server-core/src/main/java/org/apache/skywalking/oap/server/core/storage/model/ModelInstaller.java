...
public abstract class ModelInstaller implements ModelCreator.CreatingListener {
    ...
    public void whenCreating(Model model) throws StorageException {
        whenCreatingTable(model);

        // huawei update.无损演练开关：是否需要新建复制索引
        if (CoreModuleConfig.losslessDrillSwitchStatus) {
            // huawei update.无损演练：启动时添加复制标签索引
            ...
            whenCreatingTable(copyModel);
        }
    }
    private void whenCreatingTable(Model model) throws StorageException {
