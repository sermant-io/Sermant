...
import com.huawei.oap.server.configuration.zookeeper.ZKHelper;
...
public class ZookeeperConfigWatcherRegister extends ConfigWatcherRegister {
    ...
    public ZookeeperConfigWatcherRegister(ZookeeperServerSettings settings) throws Exception {
        ...
        // huawei update 增加权限配置
        CuratorFramework client;
        if (ZKHelper.isEnabledACL()) {
            client = ZKHelper.getAclZkClient(settings.getHostPort(), retryPolicy);
        } else {
            client = CuratorFrameworkFactory.newClient(settings.getHostPort(), retryPolicy);
        }
        client.start();
